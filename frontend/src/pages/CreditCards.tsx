        ...
        <IonModal isOpen={!!selectedCard} onDidDismiss={() => setSelectedCard(null)}>
          ...
          <IonContent className="ion-padding">
              ...
              {invoices.length > 0 && (
                <div style={{ minWidth: '160px' }}>
                  <IonSelect 
                    ...
                    interface="popover"
                    style={{ background: 'var(--ion-color-step-100, rgba(255,255,255,0.08))', borderRadius: '8px', padding: '4px 10px', fontSize: '13px', fontWeight: 600 }}
                  >
                    {invoices.map(inv => (
            ...
            {txLoading ? (
              ...
              <IonList className="glass-list">
                {transactions.map(tx => (
                  <IonItem key={tx.id} lines="full" style={{ '--padding-start': '8px' }}>
